...
#Boot the servers
echo openstack server create --flavor $FLAVOR_ID --image $CIRROS_ID --hint group=$ANTI_AFFINITY_ID --nic net-id=$NETWORK_ID soft-anti-affinity-server0
openstack server create --flavor $FLAVOR_ID --image $CIRROS_ID --hint group=$ANTI_AFFINITY_ID --nic net-id=$NETWORK_ID soft-anti-affinity-server0

export SERVER0_ID=`openstack server list | grep soft-anti-affinity-server0 | awk '{print $2}'`
echo
...
export SERVER1_ID=`openstack server list | grep soft-anti-affinity-server1 | awk '{print $2}'`
echo
...
#Check to make sure they're on different hypervisors
...
if [[ $S0_HYPERVISOR != $S1_HYPERVISOR ]];then
  echo "+------------------------------------------------+"
  echo "|   PRE-CHECK PASSED - HYPERVISORS DON'T MATCH   |"
  echo "+------------------------------------------------+"

else
  echo "+------------------------------------------------+"
  echo "|      PRE-CHECK FAILED - HYPERVISORS MATCH      |"
  echo "+------------------------------------------------+"
  exit 1
fi
echo
...
if [ "`openstack server list | grep soft-anti-affinity-server0`" ];then
