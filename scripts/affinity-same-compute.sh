...
export SMALL_FLAVOR_ID=`openstack flavor list | grep m1.small | awk '{print $2}'`
...
#Boot the servers
echo openstack server create --flavor $SMALL_FLAVOR_ID --image $CIRROS_ID --hint group=$AFFINITY_ID --nic net-id=$NETWORK_ID soft-affinity-server1 
openstack server create --flavor $SMALL_FLAVOR_ID --image $CIRROS_ID --hint group=$AFFINITY_ID --nic net-id=$NETWORK_ID soft-affinity-server1
echo

sleep 60

echo openstack server create --flavor $SMALL_FLAVOR_ID --image $CIRROS_ID --hint group=$AFFINITY_ID --nic net-id=$NETWORK_ID soft-affinity-server2
openstack server create --flavor $SMALL_FLAVOR_ID --image $CIRROS_ID --hint group=$AFFINITY_ID --nic net-id=$NETWORK_ID soft-affinity-server2
echo
